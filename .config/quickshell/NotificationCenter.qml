...
Variants {
  ...
  OverlayPanel {
    ...
    visible: centerPanelState

    property bool centerPanelState: false
    property bool spawning: false
    property bool closing: false

    function syncState() {
      const shouldShow = root.centerOpen && root.focusedScreen === modelData
      if (shouldShow) {
        if (!centerPanelState) {
          closeTimer.stop()
          closing = false
          centerPanelState = true
          spawnFadeIn()
        }
      } else {
        fadeClose()
    ...
    function fadeClose() {
      if (!centerPanelState || closing) return
      spawning = false
      closing = true
      centerBox.opacity = 0
      closeTimer.start()
